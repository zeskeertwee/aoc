...
fn part1(input: &Input) -> usize {
    run_through_map(input.starting_pos, &input.map).0.len()
}

#[aoc(day6, part2)]
fn part2(input: &Input) -> usize {
    let visited_positions = run_through_map(input.starting_pos, &input.map).0;

    visited_positions.into_par_iter().map(|pos| {
        let mut grid  = input.map.clone();
        grid[pos.y as usize][pos.x as usize] = '#';

        run_through_map(input.starting_pos, &grid).1 as usize
    }).sum()
}
...
fn run_through_map(starting_pos: Vector2, map: &Vec<Vec<char>>) -> (HashSet<Vector2>, bool) {
    ...
    loop {
        ...
        if map[next_position.y as usize][next_position.x as usize] == '#' {
            if !rot_pos.insert((position, direction)) {
